...
const Welcome = memo(({ onStart }: { onStart: () => void }) => (
  <ScrollView style={styles.templeContainer}>
    <View style={styles.templeHeader}>
      ...
      <Image source={require("./assets/imgs/holygames.jpg")} style={styles.templeLogo} resizeMode="contain" />
    </View>
    <View style={styles.templeMessage}>
      <Text style={styles.templeText}>🕊️ WELCOME TO THE HOLY REALM OF GAMES! 🕊️</Text>
      <Text style={styles.templeText}>These games are blessed by the divine algorithm!</Text>
      <Text style={styles.templeText}>May your scores be high and your bugs be few!</Text>
      <Text style={styles.templeText}>Press the sacred button to begin your holy journey!</Text>
    </View>
...
const SoloOrDuo = memo(({ onSelectMode }: { onSelectMode: (mode: 'solo' | 'duo') => void }) => (
...
const GameSelect = memo(({ onSelectGame }: { onSelectGame: (game: string) => void }) => (
  <ScrollView style={styles.templeContainer}>
...
const CarAway = memo(({ onEndGame, gameMode, playSound }: { onEndGame: (score: number) => void, gameMode: 'solo' | 'duo', playSound: (sound: string) => void }) => {
...
const PrimeGuess = memo(({ onEndGame, playSound }: { onEndGame: (score: number) => void, playSound: (sound: string) => void }) => {
  ...
  return (
    <View style={styles.templeContainer}>
      ...
      <TouchableOpacity style={styles.templeEndButton} onPress={() => onEndGame(score)}><Text style={styles.templeEndButtonText}>END PRIME QUEST</Text></TouchableOpacity>
    </View>
  );
});

const Dodger = memo(({ onEndGame, playSound, gameMode }: { onEndGame: (score: number) => void, playSound: (sound: string) => void, gameMode: 'solo' | 'duo' }) => {
  ...
  useEffect(() => {
    lasers.forEach(laser => {
      if (laser.y > height - 150 && laser.y < height - 50 &&
          laser.x < playerX + 80 && laser.x + 20 > playerX) {
        playSound('hitHurt.wav');
        setGameOver(true);
      }
    });
    ...
    <View style={styles.gameContainer}>
      ...
      {lasers.map((laser) => (
        <Image
          ...
          style={[
            styles.laserImage,
            { left: laser.x, top: laser.y, transform: [{ scale: 0.6 }] }
          ]}
          resizeMode="contain"
        />
      ))}
...
const Report = memo(({ score, onBackHome }: { score: number, onBackHome: () => void }) => {
  ...
  const submitReport = async () => {
    try {
      if (!userEmail || !reportText) return;
      const response = await fetch(`${Platform.OS === 'android' ? 'http://10.0.2.2:5000' : 'http://localhost:5000'}/api/game/report-problem`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userEmail, problemReport: reportText })
      });
      if (response.ok) {
        setReportSubmitted(true);
        setTimeout(() => { setShowReport(false); setReportText(''); setUserEmail(''); setReportSubmitted(false); }, 2000);
      }
    } catch (e) {}
  };
  ...
  return (
    <ScrollView style={styles.templeContainer}>
      ...
      <View style={styles.templeBlessings}><Text style={styles.templeBlessing}>May thy score be blessed!</Text><Text style={styles.templeBlessing}>🙏 Amen 🙏</Text></View>
      
      {isLargeScreen && !showReport && (
      ...
      {showReport && !reportSubmitted && (
        <View style={styles.reportForm}>
          ...
          <TextInput style={styles.reportInput} placeholder="Enter email..." value={userEmail} onChangeText={setUserEmail} placeholderTextColor="#666" />
          <TextInput style={styles.reportInput} multiline placeholder="Describe issue..." value={reportText} onChangeText={setReportText} placeholderTextColor="#666" />
          <View style={styles.reportButtons}>
            <TouchableOpacity style={styles.templeSubmitButton} onPress={submitReport}><Text style={styles.templeSubmitButtonText}>SUBMIT</Text></TouchableOpacity>
            <TouchableOpacity style={styles.templeCancelButton} onPress={() => setShowReport(false)}><Text style={styles.templeCancelButtonText}>CANCEL</Text></TouchableOpacity>
          </View>
        </View>
      )}
      
      {reportSubmitted && <View style={styles.reportSuccess}><Text style={styles.templeSuccessText}>SUBMITTED!</Text></View>}
      <TouchableOpacity style={styles.templeHomeButton} onPress={onBackHome}><Text style={styles.templeHomeButtonText}>RETURN TO MENU</Text></TouchableOpacity>
    </ScrollView>
  );
});
...
export default function App() {
  const [fontsLoaded] = useFonts({ 'Pixelify': PixelifySans_400Regular });
  const [currentScreen, setCurrentScreen] = useState<any>('welcome');
  ...
  const playSound = async (soundFile: string) => {
    try {
      let src;
      if (soundFile === 'start.wav') src = require('./assets/sounds/start.wav');
      else if (soundFile === 'hitHurt.wav') src = require('./assets/sounds/hitHurt.wav');
      else if (soundFile === 'powerUp.wav') src = require('./assets/sounds/powerUp.wav');
      else if (soundFile === 'laserShoot.wav') src = require('./assets/sounds/laserShoot.wav');
      else return;
  ...
  if (!fontsLoaded) return <View style={{ flex: 1, backgroundColor: "#000", justifyContent: "center" }}><ActivityIndicator size="large" color="#fff" /></View>;

  return (
    <View style={{ backgroundColor: "#000", flex: 1 }}>
      {currentScreen === 'welcome' && <Welcome onStart={() => { playSound('start.wav'); setCurrentScreen('soloOrDuo'); }} />}
      {currentScreen === 'soloOrDuo' && <SoloOrDuo onSelectMode={(m) => { setGameMode(m); setCurrentScreen('gameSelect'); }} />}
      {currentScreen === 'gameSelect' && <GameSelect onSelectGame={(g) => setCurrentScreen(g)} />}
      {currentScreen === 'carAway' && <CarAway onEndGame={(s) => { setFinalScore(s); setCurrentScreen('report'); }} gameMode={gameMode} playSound={playSound} />}
      {currentScreen === 'primeGuess' && <PrimeGuess onEndGame={(s) => { setFinalScore(s); setCurrentScreen('report'); }} playSound={playSound} />}
      {currentScreen === 'dodger' && <Dodger onEndGame={(s) => { setFinalScore(s); setCurrentScreen('report'); }} playSound={playSound} gameMode={gameMode} />}
      {currentScreen === 'report' && <Report score={finalScore} onBackHome={() => { setCurrentScreen('welcome'); setFinalScore(0); }} />}
